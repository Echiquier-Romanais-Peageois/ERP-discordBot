...
const lichess = async (message: Discord.Message) => {
  try {
    ...
    const status = team.map((user) => user.online ? "\:green_circle:" : "\:red_circle:");
    const usernames = team.map((user) => user.username);
    const discordPseudos = team.map((user) => storedUsers?.find((u) => u.pseudoLichess?.toLowerCase() === user.username.toLowerCase())?.pseudoLichess ?? '-');
