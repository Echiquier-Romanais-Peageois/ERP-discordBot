...
import { fetchFFEData } from '../api/ffe';
import { User } from '../mongo';

const configFFE = async (message: Discord.Message, licence: string, forUser: Discord.User, isAdmin: boolean) => {
  try {
    ...
    const otherUser = storedUsers.find((user) => user.idDiscord !== idDiscord && user.licenceFFE?.toLowerCase() === lcLicence);

    if (otherUser) {
      if (!isAdmin) {
        message.reply(`Désole, ce numéro est déjà assigné à ${otherUser.pseudoDiscord}`);
        return;
      }
    ...
    console.log(e)
    message.reply("Désole, je ne peux pas contacter le serveur FFE en ce moment !")
    return;
  }
};
